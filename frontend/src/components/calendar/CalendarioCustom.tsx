        ...
        {viewMode === "month" ? (
          <>
            ...
            <div className="text-sm font-semibold capitalize text-slate-700">
              {format(baseDate, "MMMM yyyy", { locale: pt })}
            </div>
            <button
              type="button"
              onClick={() => setBaseDate((d) => addMonths(d, 1))}
          ...
          <>
            ...
            <div className="text-sm font-semibold text-slate-700">
              Dia — {format(activeDay, "dd/MM/yyyy")}
            </div>
            <button
              type="button"
              onClick={() => onSelectDay?.(addDays(activeDay, 1))}
          ...
          <>
            <button
              ...
              className={`rounded border px-2 py-1 text-sm ${
                canGoPrevWeek ? "bg-white hover:bg-slate-100" : "cursor-not-allowed bg-slate-100 text-slate-400"
              }`}
            >
              {"<"}
            </button>
            <div className="text-sm font-semibold text-slate-700">
              Semana — {format(baseDate, "dd/MM/yyyy")}
            </div>
            <button
              type="button"
